...
{
	if (key == W_KEY && check_collision
		(game->map[game->p_pos.y - 1][game->p_pos.x], game))
		player_move(game, 0, -1);
	else if (key == S_KEY && check_collision
		(game->map[game->p_pos.y + 1][game->p_pos.x], game))
		player_move(game, 0, 1);
	else if (key == A_KEY && check_collision
		(game->map[game->p_pos.y][game->p_pos.x - 1], game))
		player_move(game, -1, 0);
	else if (key == D_KEY && check_collision
		(game->map[game->p_pos.y][game->p_pos.x + 1], game))
		player_move(game, +1, 0);
	else if (key == ESC_KEY)
		free_all(game);
